...
//  Versión: 1.0 MVP
//  Desplegar como: Web App → Cualquiera (anyone) puede acceder
// ============================================================

// ── CONFIGURACIÓN GLOBAL ────────────────────────────────────
const CONFIG = {
  SPREADSHEET_ID: '17EyfbaA00ff9x3lGIdlY6gb5i3D-pi29pCVPoHMhFVY', // ← Reemplazar
  PIN_ADMIN: '1234',                          // ← Cambiar en producción
  NOMBRE_EMPRESA: 'Mi Empresa',
  MAX_HISTORIAL: 30,                          // Últimos N registros por empleado
  HOJAS: {
    EMPLEADOS: 'Empleados',
    REGISTROS: 'Registros',
    AUDITORIA: 'Auditoria'
...
function inicializarHoja(hoja, nombre) {
  const headers = {
    [CONFIG.HOJAS.EMPLEADOS]: [
      'ID','Nombre','PIN','Activo','FechaAlta','Email'
    ],
...
function auditLog(accion, idEmpleado, detalle, ip) {
  try {
    ...
    // Silencioso: el log no debe bloquear la operación principal
...
function accionFichar(body, ip) {
  ...
  // 2. Determinar tipo de fichaje (ENTRADA o SALIDA)
  ...
  if (!ultimoReg) {
    // Primera acción del día → ENTRADA
    tipo     = 'ENTRADA';
    sesionID = generarID();
  } else if (ultimoReg.tipo === 'ENTRADA') {
    // Tenía entrada abierta → SALIDA
    tipo     = 'SALIDA';
  ...
  const fila = [
    ...
    empleado.nombre,
    tipo,
    ahora,                           // Timestamp servidor (fuente de verdad)
...
function accionAdminDia(params) {
  ...
  // Ordenar por timestamp servidor descendente
...
function accionAdminAbiertos(params) {
  ...
  // Mapa empleado → último registro de hoy
  const mapaUltimo = {};
  for (let i = 1; i < datos.length; i++) {
    const reg = datos[i];
    if (reg[6] === hoyStr) {
      mapaUltimo[reg[1]] = { tipo: reg[3], nombre: reg[2], hora: formatHora(reg[4]), timestamp: reg[4] };
    }
  }
...
function accionNuevoEmpleado(body) {
  const { pinAdmin, nombre, pin, email } = body;
  if (!verificarAdmin(pinAdmin)) return respErr('Acceso denegado');
  ...
  hoja.appendRow([id, nombre, pin, true, new Date(), email || '']);

  auditLog('NUEVO_EMPLEADO', 'ADMIN', { id, nombre }, '');
...
function formatFecha(date) {
  // Devuelve YYYY-MM-DD en zona horaria local del script (Madrid)
...
// ============================================================
//  FUNCIÓN DE SETUP INICIAL — Ejecutar UNA VEZ manualmente
// ============================================================
function setupInicial() {
  ...
  hEmpleados.appendRow(['EMP001', 'Empleado Demo', '1111', true, new Date(), 'demo@empresa.com']);
